...
// SEO 和 Analytics 设置
const { trackEvent, trackPageView, setSeo } = useSeo({
...
const handleVideoError = () => {
  ...
  const downloadFromPreview = async () => {
    ...
    showToast(t('toast.download_preparing'), 'info');

    const urls = previewData.value.tunnel || [];
    if (urls.length === 0) {
      showToast(t('toast.no_downloadable_links'), 'error');
      isDownloading.value = false;
      return;
    }
    ...
    try {
      for (let i = 0; i < urls.length; i++) {
        ...
        const fileType = i === 0 ? t('preview.video_file') : t('preview.audio_file');
        ...
        if (isTunnel) {
          ...
          if (!response.ok) throw new Error(`${t('toast.download_failed')}: ${response.status} ${response.statusText}`);
      ...
      showToast(t('toast.download_started_multiple', { count: urls.length }), 'success');
      closePreview();

    } catch (error) {
      console.error('❌ 下载失败:', error);
      showToast(error instanceof Error ? error.message : t('toast.download_error_unknown'), 'error');
    } finally {
      isDownloading.value = false;
    }
  };
...
const openInNewTab = () => {
  ...
  showToast(t('toast.opened_in_new_tab'), 'info')
}
...
// 下载picker选中的项目
  const downloadSelectedItems = async () => {
    ...
    try {
      showToast(t('toast.batch_download_started', { count: selectedItemsData.length }), 'info')
      ...
      for (let i = 0; i < selectedItemsData.length; i++) {
        ...
        try {
          ...
          const errorMessage = error instanceof Error ? error.message : t('toast.download_failed')
          showToast(t('toast.download_file_failed', { index: i + 1, message: errorMessage }), 'error')
        }
      }
      
      showToast(t('toast.batch_download_all_started'), 'success')
      closePicker()
      
    } catch (error) {
      console.error('❌ 批量下载过程出错:', error)
      const errorMessage = error instanceof Error ? error.message : t('toast.download_failed')
      showToast(t('toast.download_failed_with_message', { message: errorMessage }), 'error')
    } finally {
      isPickerDownloading.value = false
    }
  }
...
async function processQueueItem(item: QueuedItem) {
  try {
    ...
    item.currentStep = t('queue.analyzing_link');
    ...
    console.log('🔍 [processQueueItem] 前端合并检查:', {
      ...
      item.currentStep = t('queue.detected_separate_streams', { service });
      ...
      if (!videoUrl || !audioUrl) {
        throw new Error(t('queue.stream_url_fetch_failed'));
      }
      
      showToast(t('toast.browser_downloading_streams'), 'info');
      
      item.currentStep = t('queue.downloading_video_stream');
      console.log('📥 开始下载视频流...', {
      ...
      item.currentStep = t('queue.downloading_audio_stream');
      console.log('📥 开始下载音频流...');
      ...
      if (!videoResp.ok || !audioResp.ok) {
        ...
        throw new Error(t('queue.stream_download_failed', { video_status: videoResp.status, audio_status: audioResp.status }));
      }
      
      item.currentStep = t('queue.processing_video_data');
      console.log('📦 转换为Blob对象...');
      showToast(t('toast.browser_parsing_streams'), 'info');
      
      // 分步处理，避免UI阻塞
      console.log('📦 处理视频流...');
      const videoBlob = await videoResp.blob();
      
      item.currentStep = t('queue.processing_audio_data');
      console.log('📦 处理音频流...');  
      const audioBlob = await audioResp.blob();
      
      item.currentStep = t('queue.data_processing_complete');
      console.log('✅ 流数据处理完成');
      ...
      showToast(t('toast.browser_merging_streams', { service }), 'info');
      
      item.currentStep = t('queue.initializing_video_engine');
      console.log('🔄 调用remux函数...');
      showToast(t('toast.initializing_video_engine'), 'info');
      try {
        const mergedBlob = await remux(videoBlob, audioBlob, (step: string) => {
          item.currentStep = step; // remux函数中会传递步骤信息
        });
        
        item.currentStep = t('queue.merge_complete_preparing_download');
        console.log('✅ remux合并完成，文件大小:', (mergedBlob.size / 1024 / 1024).toFixed(2) + 'MB');
        showToast(t('toast.browser_merge_complete'), 'success');
        
        const filename = item.response.filename || `${service}_merged.mp4`;
        
        item.currentStep = t('queue.starting_download');
        console.log(`✅ ${service}视频合并完成，开始下载:`, filename);
        downloadFile(mergedBlob, filename);
        
        item.status = 'done';
        item.currentStep = t('queue.download_complete');
        ...
        if (videoBlob.size > 0 && audioBlob.size > 0) {
          // 如果文件下载成功但合并失败，分别下载
          showToast(t('toast.browser_merge_failed_downloading_separately'), 'warning');
          
          item.currentStep = t('queue.merge_failed_downloading_separately');
          console.log('🔄 启用降级方案：分别下载视频和音频文件');
          
          const baseFilename = item.response.filename?.replace(/\.[^/.]+$/, '') || `${service}_video`;
          
          // 下载视频文件
          item.currentStep = t('queue.downloading_video_file');
          downloadFile(videoBlob, `${baseFilename}_video.mp4`);
          
          // 下载音频文件  
          item.currentStep = t('queue.downloading_audio_file');
          downloadFile(audioBlob, `${baseFilename}_audio.m4a`);
          
          item.status = 'done';
          item.currentStep = t('queue.downloaded_separately');
          showToast(t('toast.downloaded_separately'), 'success');
          console.log(`🎉 ${service}视频降级下载完成！`);
          
        } else {
          // 文件下载就失败了
          showToast(t('toast.stream_download_failed_error'), 'error');
          item.status = 'error';
          item.currentStep = t('queue.stream_download_failed_error');
        }
      }
      ...
      item.currentStep = t('queue.downloading_multiple_files');
      const tunnelUrls = (item.response as any).tunnel;
      
      for (let i = 0; i < tunnelUrls.length; i++) {
        item.currentStep = t('queue.downloading_file_number', { index: i + 1, total: tunnelUrls.length });
        const url = tunnelUrls[i];
        const response = await fetch(url);
        if (!response.ok) throw new Error(t('queue.download_stream_failed', { index: i + 1 }));
        const blob = await response.blob();
      ...
      item.status = 'done';
      item.currentStep = t('queue.all_files_downloaded');
      
    } else if (item.response.url) {
      // 处理单文件下载
      console.log('📁 检测到单文件下载...');
      item.currentStep = t('queue.downloading_file');
      const response = await fetch(item.response.url);
      if (!response.ok) throw new Error(t('queue.file_download_failed'));
      
      item.currentStep = t('queue.processing_file_data');
      const blob = await response.blob();
      ...
      throw new Error(t('queue.no_downloadable_links'));
    }
    ...
    item.status = 'error';
    item.currentStep = t('queue.processing_failed') + (error instanceof Error ? error.message : t('queue.unknown_error'));
    showToast(error instanceof Error ? error.message : t('toast.queue_processing_failed'), 'error');
  }
}
...
function addToQueue({ response, request }: { response: any, request: any }) {
  ...
  if (!enhancedFilename || enhancedFilename.trim() === '') {
    ...
    if (!enhancedFilename || enhancedFilename.trim() === '') {
      const service = response.service || t('queue.unknown_platform');
    ...
    <div class="relative z-10 flex flex-col min-h-screen pb-safe">
      ...
      <header class="px-6 py-6 relative">
        ...
        >
          ...
          <span class="hidden sm:inline text-sm sm:text-base">{{ $t('settings') }}</span>
        </button>
        ...
        <div class="max-w-4xl mx-auto pt-24 text-center px-4">
          ...
          <p class="text-slate-300 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed mt-6">
            {{ $t('header.subtitle') }}
            <br>
            <span class="text-sm sm:text-base text-slate-400 font-medium">{{ $t('header.subtitle_highlight') }}</span>
          </p>
        </div>
      </header>
          ...
          <div class="flex justify-center">
            ...
            >
              ...
              <div class="flex items-center space-x-2 px-4 py-2 bg-slate-800/50 border border-white/10 rounded-full 
                ...
                <span class="text-sm text-slate-300 group-hover:text-white">{{ $t('supported_platforms') }}</span>
              ...
              >
                <div class="text-center mb-3">
                  <h4 class="text-white font-semibold text-sm">{{ $t('supported_platforms') }}</h4>
                </div>
      ...
      <footer class="px-6 py-6 text-center">
        <div class="max-w-4xl mx-auto">
          ...
          <div class="text-sm text-slate-400 space-y-1">
            ...
            <div>
              <span>{{ $t('footer.built_with_pre') }}</span>
              <a 
              ...
              <span>{{ $t('footer.built_with_post') }}</span>
            </div>
          </div>
        </div>
      </footer>
    ...
    >
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-lg font-bold text-white">
          {{ $t('queue.title') }} <span class="text-xs align-top bg-blue-500/80 text-white px-1.5 py-0.5 rounded-full">BETA</span>
        </h3>
        <button
        ...
        <div v-for="item in processingQueue" :key="item.id" class="bg-slate-900/70 p-3 rounded-lg">
          ...
          <div class="flex items-center justify-between mb-2">
            <p 
              ...
              :title="item.response.filename || $t('queue.source', { service: (item.response as any).service || $t('queue.unknown_platform') })"
              :class="{ 'text-gray-400': !item.response.filename }"
            >
              {{ item.response.filename || $t('queue.media_file', { service: (item.response as any).service || $t('queue.unknown_platform') }) }}
            </p>
            <!-- 状态图标 -->
          ...
          <div class="flex items-center space-x-2">
            ...
            <p class="text-xs flex-1" :class="{
              ...
              {{ item.currentStep || (item.status === 'queued' ? $t('queue.waiting_for_processing') : item.status + '...') }}
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-6">
          ...
          <div v-if="previewData" class="text-center">
            <h4 class="text-lg font-semibold text-white mb-2">
              {{ previewData.filenameAttributes?.title || $t('preview.unknown_title') }}
            </h4>
            <p class="text-gray-400 text-sm">
              {{ previewData.filenameAttributes?.author || $t('preview.unknown_author') }}
            </p>
          ...
          <div v-if="previewData?.tunnel?.[0]" class="aspect-video bg-black rounded-lg overflow-hidden relative">
            ...
            <div v-if="videoError" class="w-full h-full flex items-center justify-center bg-slate-800/50">
              <div class="text-center text-gray-400">
                ...
                <p class="text-sm text-white font-medium">
                  <span v-if="previewData.tunnel[0].includes('/tunnel?')">{{ $t('preview.proxy_video_failed') }}</span>
                  <span v-else>{{ $t('preview.preview_unavailable') }}</span>
                </p>
                <p class="text-xs mt-1 text-gray-300">
                  <span v-if="previewData.tunnel[0].includes('/tunnel?')">{{ $t('preview.proxy_video_unavailable_desc') }}</span>
                  <span v-else>{{ $t('preview.preview_unavailable_desc') }}</span>
                </p>
                <p class="text-xs text-pink-300 mt-2">{{ $t('preview.download_instead') }}</p>
              </div>
            </div>
            ...
            <div v-if="!videoError" class="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded">
              <span v-if="previewData.tunnel[0].includes('/tunnel?')">
                {{ $t('preview.loading_proxy') }}
              </span>
              <span v-else>
                {{ $t('preview.loading_video') }}
              </span>
            </div>
          </div>
          ...
          <div class="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4">
            <h5 class="text-yellow-400 font-semibold mb-2">📁 {{ $t('preview.file_info') }}</h5>
            
            <!-- 合并类型说明 -->
            <div v-if="previewData.type === 'merge'" class="text-gray-300 text-sm mb-3">
              <p class="mb-2">{{ $t('preview.merge_info') }}</p>
              <ul class="list-disc list-inside ml-2 space-y-1">
                <li><strong>{{ $t('preview.video_file') }}</strong>{{ $t('preview.video_file_desc') }}</li>
                <li><strong>{{ $t('preview.audio_file') }}</strong>{{ $t('preview.audio_file_desc') }}</li>
              </ul>
              <p class="mt-2 text-yellow-300">{{ $t('preview.merge_required') }}</p>
            </div>
            
            <!-- 单文件说明 -->
            <div v-else-if="previewData.type === 'single' || previewData.type === 'redirect'" class="text-gray-300 text-sm mb-3">
              <p>✅ {{ $t('preview.single_file_detected') }}</p>
              <p class="text-green-300 text-xs mt-1">{{ $t('preview.single_file_desc') }}</p>
            </div>
            
            <!-- 多文件说明 -->
            <div v-else class="text-gray-300 text-sm mb-3">
              <p>📦 {{ $t('preview.multiple_files_detected') }}</p>
              <p class="text-blue-300 text-xs mt-1">{{ $t('preview.file_count') }}: {{ previewData.tunnel?.length || 0 }}</p>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row justify-center gap-3">
            ...
            >
              ...
              <span v-else-if="previewData.type === 'merge'">{{ $t('preview.download_separated_files') }}</span>
              <span v-else-if="previewData.type === 'single' || previewData.type === 'redirect'">{{ $t('preview.download_file') }}</span>
              <span v-else>{{ $t('preview.download_all_files', { count: previewData.tunnel?.length || 0 }) }}</span>
            </button>
      ...
      <div class="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-8 m-4">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-gray-400 text-sm mt-1">{{ $t('picker.found_files', { count: pickerData?.picker?.length || 0 }) }}</p>
          </div>
          <button
            @click="closePicker"
            class="p-2 rounded-lg hover:bg-white/10 transition-colors text-gray-400 hover:text-white"
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg border border-slate-600">
            <div class="flex items-center space-x-3">
              ...
              <span class="text-white font-medium">
                {{ selectedItems.size === pickerData?.picker?.length ? $t('picker.deselect_all') : $t('picker.select_all') }}
              </span>
            </div>
            <span class="text-gray-400 text-sm">
              {{ $t('picker.selected_files', { selected: selectedItems.size, total: pickerData?.picker?.length || 0 }) }}
            </span>
          </div>
          ...
          <div class="flex justify-center pt-4">
            ...
            >
              ...
              <span v-else>{{ $t('picker.download_selected', { count: selectedItems.size }) }}</span>
            </button>
          </div>
        </div>
      </div>
